#!/usr/bin/env python
# Original from: https://github.com/leebaird/discover
        ...
        for port_details in info_detail:
            ...
            for port_services in port_service:
                ...
                for service_details in service_info:
                    ...
                    yield(port_number,port_state,protocol,service,product,version)

    def _parse_XML_details(self,fileExists):
        '''Initiate parsing of nmap XML file and create CSV string object'''
        csv_header = "IP Address,FQDN,OS,Port,Port Status,Protocol,Service,Name,Version\n"
        csv_format = '{0},"{1}","{2}",{3},{4},"{5}","{6}","{7}","{8}"\n'
        ...
        for info in self._iter_hosts():
            ...
            for port,port_status,protocol,service,product,version in self._get_iter_Port_Information(info):
                self._csv_string += csv_format.format(ip,fqdn,os,port,port_status,protocol,service,product,version)

    def dumpCSV(self):
        '''Write CSV output file to disk'''

        if os.path.isfile(self._output_path):
            self._parse_XML_details(True)
            csv_output = open(self._output_path,"a")
        else:
            self._parse_XML_details(False)
...
if __name__ == "__main__":
    ...
    for filename in os.listdir("."):
        ...
        nmap_xml = NMAP_XMLParser(filename)          # Input file
        nmap_xml.setCSVPath("namp.csv")                # Output file
        nmap_xml.dumpCSV()
